...
const validateComponentProps = (props: MarkdownProps) => {
  if (props.value && typeof props.value !== "string") {
    throw new TypeError(
      `[marked-react]: Expected value to be of type string but got ${typeof props.value}`,
    );
  }

  if (props.children && typeof props.children !== "string") {
    throw new TypeError(
      `[marked-react]: Expected children to be of type string but got ${typeof props.children}`,
    );
  }
};
...
const Markdown = (props: MarkdownProps) => {
  ...
  const children = options.isInline
