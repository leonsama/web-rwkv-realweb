...
class ReactRenderer {
  ...
  constructor(options: ReactRendererOptions = {}) {
    const { renderer, keyPrefix } = options;

    this.options = options;
    this.keyPrefix = keyPrefix || "md";
    ...
    if (renderer && typeof renderer === "object") {
      Object.entries(renderer as ReactRenderer).forEach(([key, value]) => {
        ...
        if (
          ...
          rendererName === "keyPrefix" ||
          rendererName === "options" ||
  ...
  #h<T extends ElementType>(
    ...
    const elProps = {
      key: `${this.keyPrefix}-${this.elementId}`,
    };
  ...
  get elementId() {
    return `${this.keyPrefix}-${this.elIdList.join("-")}`;
  }
